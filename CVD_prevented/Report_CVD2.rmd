...
print(xtable(res,digits=c(0,0,0,3,3,3),
      caption = "prevented CVD events under the screening of Strategy 2"),
      caption.placement="top")
```


```{r}
population_data_strategy2 <- population_data[which(population_data$strategy=="strategy2"),
                                             c("low","medium","high")]
cvd_dat_strategy2 <- data.frame(cbind(as.numeric(as.character(res$low)),
                            as.numeric(as.character(res$medium)),
                            as.numeric(as.character(res$high))))
result_strategy2 <- sum(population_data_strategy2*cvd_dat_strategy2)
result_strategy2
```
