...
for (g in 1:n_loop){
  ...
  ##2.1 HR
  set.seed(+g)
  HR_cvdhistory_cvd<-rnorm(1,1.37,0.025)  ##cvd病史复发cvd的HR
  HR_cvdhistory_cvdth<-rnorm(1,3.12,0.098)  ##cvd病史死亡的HR
  HR_high_live_cvdth<-rnorm(1,1.17,0.0204)  ##高风险人群死于cvd的HR
  
  HR_l_stg1<-rnorm(1,0.63,0.041)  ##策略1低危人群的发病HR
  HR_m_stg1<-rnorm(1,1.56,0.117)  ##策略1中危人群的发病HR
  HR_h_stg1<-rnorm(1,1.6,0.102)   ##策略1高危人群的发病HR
  
  HR_l_stg2<-rnorm(1,0.43,0.0357)  ##策略2低危人群的发病HR
  HR_m_stg2<-rnorm(1,0.97,0.0663)  ##策略2中危人群的发病HR
  HR_h_stg2<-rnorm(1,2.06,0.117)  ##策略2高危人群的发病HR
  
  HR_l_stg3<-rnorm(1,0.45,0.0357)  ##策略3低危人群的发病HR
  HR_m_stg3<-rnorm(1,1.09,0.0765)  ##策略3中危人群的发病HR
  HR_h_stg3<-rnorm(1,2.11,0.122)  ##策略3高危人群的发病HR
  
  
  ##2.2输入干预策略的HR
  HR_smk_cvd<-rnorm(1,0.85,0.016)   ##戒烟对cvd发病的影响
  HR_smk_cvdth<-rnorm(1,0.72,0.131)  ##戒烟对cvd死亡的影响
  HR_salt_cvd<-rnorm(1,0.81,0.08)
  HR_salt_cvdth<-rnorm(1,0.66,0.144)
  HR_wtc_cvd<-rnorm(1,0.93,0.05)
  HR_wtc_cvdth<-rnorm(1,0.93,0.16)
  HR_hpt_lip_cvd<-rnorm(1,0.7,0.17)
  HR_hpt_lip_cvdth<-rnorm(1,0.82,0.13)
  
  p8_min<-0.0203  
  p8_max<-0.096  ##急性期死亡的均匀分布下限和上限
...
p5_0<-ProbFactor(p2_0,HR_cvdhistory_cvdth)  ##有cvd病史的人死亡的概率
p7_0<-ProbFactor(p1_0,HR_cvdhistory_cvd)  ##有cvd病史复发cvd的概率
...
#策略2：
population_stg2<-array(NA,dim = c(n_population,n_risk),dimnames = list(names_population,names_risk))
population_stg2[,1]<-c(163,212,106,35,5,1,0,478,734,537,277,44,4,0) ##低危人群，前7位男性，后7位为女性
population_stg2[,2]<-c(50,168,231,210,81,15,3,17,108,238,433,282,104,23) ##中危人群，前7位男性，后7位为女性
population_stg2[,3]<-c(16,64,85,165,236,144,113,3,14,47,132,193,286,164) ##高危人群，前7位男性，后7位为女性
...
N_cvd_stg1[g]<-sum(n_cvd_stg)
N_cd_stg1[g]<-sum(n_cd_stg)
N_nd_stg1[g]<-sum(n_nd_stg)
N_d_stg1[g]<-sum(n_d_stg)
QALY_stg1[g]<-sum(qaly_stg)
LIFEYEAR_stg1[g]<-sum(lifeyear_stg)
...
HR_h_stg<-HR_h_stg2
#不同策略的低中高危人群分布不同
population_stg<-population_stg2
###运行程序
p1_0_stg<-matrix(0,nrow=n_risk,ncol=n_population,dimnames=list(names_risk,names_population),byrow=F)  ##低中高危人群发生cvd的概率
p1_0_stg[1,]<-ProbFactor(p1_0,HR_l_stg)  ##不同策略筛出的低危人群有不同的发病概率
p1_0_stg[2,]<-ProbFactor(p1_0,HR_m_stg)  ##不同策略筛出的中危人群有不同的发病概率
...
N_cvd_stg2[g]<-sum(n_cvd_stg)
N_cd_stg2[g]<-sum(n_cd_stg)
N_nd_stg2[g]<-sum(n_nd_stg)
N_d_stg2[g]<-sum(n_d_stg)
QALY_stg2[g]<-sum(qaly_stg)
LIFEYEAR_stg2[g]<-sum(lifeyear_stg)
...
N_cvd_stg3_1[g]<-sum(n_cvd_stg)
N_cd_stg3_1[g]<-sum(n_cd_stg)
N_nd_stg3_1[g]<-sum(n_nd_stg)
N_d_stg3_1[g]<-sum(n_d_stg)
QALY_stg3_1[g]<-sum(qaly_stg)
LIFEYEAR_stg3_1[g]<-sum(lifeyear_stg)
...
p5_0<-ProbFactor(p2_0,HR_cvdhistory_cvdth)  ##有cvd病史的人死亡的概率
p7<-ProbFactor(p1_0,HR_cvdhistory_cvd)  ##有cvd病史复发cvd的概率
...
set.seed(1)
p8<-array(runif(140,min=p8_min,max=p8_max),dim=c(10,n_population),dimnames=list(1:10,names_population))  ##cvd发作急性死亡概率
...
N_cvd_stg3_2[g]<-sum(n_cvd)
N_cd_stg3_2[g]<-sum(n_cd)
N_nd_stg3_2[g]<-sum(n_nd)
N_d_stg3_2[g]<-sum(n_d)
QALY_stg3_2[g]<-sum(qaly)
LIFEYEAR_stg3_2[g]<-sum(lifeyear)

N_cvd_stg3[g]<-N_cvd_stg3_1[g]+N_cvd_stg3_2[g]
N_cd_stg3[g]<-N_cd_stg3_1[g]+N_cd_stg3_2[g]
N_nd_stg3[g]<-N_nd_stg3_1[g]+N_nd_stg3_2[g]
N_d_stg3[g]<-N_d_stg3_1[g]+N_d_stg3_2[g]
QALY_stg3[g]<-QALY_stg3_1[g]+QALY_stg3_2[g]
LIFEYEAR_stg3[g]<-LIFEYEAR_stg3_1[g]+LIFEYEAR_stg3_2[g]
}
...
for (g in 1:n_loop){
  ...
  N_cvd_stg0[g]<-sum(n_cvd)
  N_cd_stg0[g]<-sum(n_cd)
  N_nd_stg0[g]<-sum(n_nd)
  N_d_stg0[g]<-sum(n_d)
  QALY_stg0[g]<-sum(qaly)
  LIFEYEAR_stg0[g]<-sum(lifeyear)
  
  #N_cvd_stg0 ##总人群cvd发病数
  #N_cd_stg0 ##总人群因cvd死亡数
  #N_nd_stg0 ##总人群非cvd死亡数
  #N_d_stg0 ##总人群全死因死亡数
  #QALY_stg0 ##总人群qaly
  #LIFEYEAR_stg0 ##总人数获得生命年
}


write.csv(N_cvd_stg1,"N_cvd_stg1_psaoutcome.csv")
write.csv(N_cvd_stg2,"N_cvd_stg2_psaoutcome.csv")
write.csv(N_cvd_stg3,"N_cvd_stg3_psaoutcome.csv")
write.csv(N_cvd_stg0,"N_cvd_stg0_psaoutcome.csv")
write.csv(N_cd_stg0,"N_cd_stg0_psaoutcome.csv")
write.csv(N_cd_stg1,"N_cd_stg1_psaoutcome.csv")
write.csv(N_cd_stg2,"N_cd_stg2_psaoutcome.csv")
write.csv(N_cd_stg3,"N_cd_stg3_psaoutcome.csv")
write.csv(N_d_stg0,"N_d_stg0_psaoutcome.csv")
write.csv(N_d_stg1,"N_d_stg1_psaoutcome.csv")
write.csv(N_d_stg2,"N_d_stg2_psaoutcome.csv")
write.csv(N_d_stg3,"N_d_stg3_psaoutcome.csv")
write.csv(QALY_stg0,"QALY_stg0_psaoutcome.csv")
write.csv(QALY_stg1,"QALY_stg1_psaoutcome.csv")
write.csv(QALY_stg2,"QALY_stg2_psaoutcome.csv")
write.csv(QALY_stg3,"QALY_stg3_psaoutcome.csv")
write.csv(LIFEYEAR_stg0,"N_cvd_stg0_psaoutcome.csv")
write.csv(LIFEYEAR_stg1,"N_cvd_stg1_psaoutcome.csv")
write.csv(LIFEYEAR_stg2,"N_cvd_stg2_psaoutcome.csv")
write.csv(LIFEYEAR_stg3,"N_cvd_stg3_psaoutcome.csv")
...
for (g in 1:n_loop) {
##最少发病
  if (min(N_cvd_stg1[g],N_cvd_stg2[g],N_cvd_stg3[g],N_cvd_stg0[g])==N_cvd_stg0[g]){
    best_cvd[g]<-0
  } else if (min(N_cvd_stg1[g],N_cvd_stg2[g],N_cvd_stg3[g],N_cvd_stg0[g])==N_cvd_stg1[g]){
  best_cvd[g]<-1
} else if (min(N_cvd_stg1[g],N_cvd_stg2[g],N_cvd_stg3[g],N_cvd_stg0[g])==N_cvd_stg2[g]) {
  best_cvd[g]<-2
} else best_cvd[g]<-3

##最少因cvd死亡
if (min(N_cd_stg1[g],N_cd_stg2[g],N_cd_stg3[g],N_cd_stg0[g])==N_cd_stg0[g]){
  best_cd[g]<-0
} else if (min(N_cd_stg1[g],N_cd_stg2[g],N_cd_stg3[g],N_cd_stg0[g])==N_cd_stg1[g]){
  best_cd[g]<-1
} else if (min(N_cd_stg1[g],N_cd_stg2[g],N_cd_stg3[g],N_cd_stg0[g])==N_cd_stg2[g]) {
  best_cd[g]<-2
} else best_cd[g]<-3

##最少全死因死亡
if (min(N_d_stg1[g],N_d_stg2[g],N_d_stg3[g],N_d_stg0[g])==N_d_stg0[g]){
  best_d[g]<-0
} else if (min(N_d_stg1[g],N_d_stg2[g],N_d_stg3[g],N_d_stg0[g])==N_d_stg1[g]){
  best_d[g]<-1
} else if (min(N_d_stg1[g],N_d_stg2[g],N_d_stg3[g],N_d_stg0[g])==N_d_stg2[g]){
  best_d[g]<-2
} else best_d[g]<-3

##最多qaly
if (max(QALY_stg1[g],QALY_stg2[g],QALY_stg3[g],QALY_stg0[g])==QALY_stg0[g]){
  best_qaly[g]<-0
} else if (max(QALY_stg1[g],QALY_stg2[g],QALY_stg3[g],QALY_stg0[g])==QALY_stg1[g]){
  best_qaly[g]<-1
} else if (max(QALY_stg1[g],QALY_stg2[g],QALY_stg3[g],QALY_stg0[g])==QALY_stg2[g]) {
  best_qaly[g]<-2
} else{
  best_qaly[g]<-3
}
##最多生命年
if (max(LIFEYEAR_stg1[g],LIFEYEAR_stg2[g],LIFEYEAR_stg2[g],LIFEYEAR_stg0[g])==LIFEYEAR_stg1[g]){
  best_lifeyear[g]<-0
}else if (max(LIFEYEAR_stg1[g],LIFEYEAR_stg2[g],LIFEYEAR_stg2[g],LIFEYEAR_stg0[g])==LIFEYEAR_stg1[g]){
  best_lifeyear[g]<-1
} else if (max(LIFEYEAR_stg1[g],LIFEYEAR_stg2[g],LIFEYEAR_stg2[g],LIFEYEAR_stg0[g])==LIFEYEAR_stg2[g]) {
  best_lifeyear[g]<-2
}else best_lifeyear[g]<-3
}
...
###2.计算NNT和95%CI
##95%CI
quantile(QALY_stg0,c(0.025,0.975))  ##(58908.60,58925.01)
quantile(QALY_stg1,c(0.025,0.975))  ##()
quantile(QALY_stg2,c(0.025,0.975))
quantile(QALY_stg3,c(0.025,0.975))


##计算NNT
NNT_cvd_stg1<-0  #策略1每预防一例cvd发生数需要筛查的人数
for (g in 1:n_loop){
  NNT_cvd_stg1[g]<- sum(population_stg1)/(N_cvd_stg0[g]-N_cvd_stg1[g]) 
}
quantile(NNT_cvd_stg1,c(0.025,0.975))  (21.2,71.8)
