...
N_cvd_stg2<-sum(n_cvd_stg)
N_cd_stg2<-sum(n_cd_stg)
N_nd_stg2<-sum(n_nd_stg)
N_d_stg2<-sum(n_d_stg)
QALY_stg2<-sum(qaly_stg)
LIFEYEAR_stg2<-sum(lifeyear_stg)

N_cvd_stg2 ##总人群cvd发病数
N_cd_stg2 ##总人群因cvd死亡数
N_nd_stg2 ##总人群非cvd死亡数
N_d_stg2 ##总人群全死因死亡数yy
QALY_stg2 ##总人群qaly
LIFEYEAR_stg2  ##总人数获得生命年