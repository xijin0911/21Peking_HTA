...
mat <- matrix(NA, nrow=nrow(m_M_corr),ncol = 4)
for (j in 1:nrow(m_M_corr)){
  mat[j,] <- uti_values
}
QALY_l <- sum(m_M_corr*mat)  # is the calculated QALY for the assumed patient under low level
print(m_M_corr*mat)
...
QALY_m <- sum(m_M_corr*mat)  # is the calculated QALY for the assumed patient under medium level
```
...
QALY_h <- sum(m_M_corr*mat)  # is the calculated QALY for the assumed patient under high level
```
...
To calculate the require QALY for both male and female patients under different ages, construct a vector to and run a loop save the calculated result 
```{r}
